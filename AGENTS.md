...
Boneyard skeleton artifacts live here. `registry.ts` is generated by
`npx boneyard-js build` and is imported for side effects from `src/main.ts`; do not edit it by hand.
Keep skeleton names in templates synchronized with the registered names and their `.bones.json`
files.
...
- Render or propagate resource loading and error states. Page-wide Boneyard skeletons aggregate
  child loading outputs into page signals.
